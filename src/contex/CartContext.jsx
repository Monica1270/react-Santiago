...
/* import { productos } from "../mok/AsyncService"; */
    ...
    //es una funcion dinamica y le estoy dicendo que reciba los parametros item y qty que significa quantity
        ...
        const addItem = (item, qty) =>{
            if (isInCart(item.id)){
                ...
                const carritoActualizado = cart.map((productos) => {
        ...
        const total = () => {
            return cart.reduce((acc, productos) => (acc += productos.quantity * productos.price), 0)
        }
        //calcular cantidad
        const cartQuantity = () => {
            return cart.reduce((acc, productos) => acc += productos.quantity, 0)
        }
